...
- Auth: token simples em memória com expiração de 30 dias, senha única familiar
- Sem WebSocket — polling 15s no frontend para itens, categorias e configurações (pausa quando aba oculta ou drawer de quantidade aberto)
...
│   ├── models.py
│   ├── schemas.py
│   ├── seed.py          # bootstrap dos 4 usuários + 11 categorias + ~270 itens
│   ├── auth.py
│   └── routers/
│       ├── auth.py
│       ├── items.py
...
Senha única para todos: `Aa123456` (definida em `backend/auth.py`).

## Como usar

- **Login:** clica no nome, digita a senha.
- **Marcar/desmarcar item:** toque simples em item inativo ativa o item. Para desativar um item ativo, use duplo toque/clique. Ao desativar, a quantidade é zerada.
...
- **Índice alfabético:** botão `A-Z` alterna uma barra lateral para navegar rapidamente pelos itens inativos.

## Notas

- Polling automático a cada 15s — atualiza itens, categorias e configurações; pausa quando a aba está oculta ou o drawer de quantidade está aberto.
...
- CORS aceita rede local por padrão. Para liberar um hostname público direto no backend, defina `XGROCERY_PUBLIC_ORIGIN_REGEX`.
